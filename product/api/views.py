...
from product.api.serializers import ProductCategorySerializer, ProductSerializer, ProductCreateSerializer, ProductTagSerializer
...
def product_update(request, pk):
    ...
    if request.method == 'PATCH':
        product = Product.objects.get(id = pk)
        serializer_class = ProductCreateSerializer(data = request.data, instance = product, partial = True)
        if serializer_class.is_valid():
            serializer_class.save()
            return JsonResponse(data = serializer_class.data, safe = False)
        return JsonResponse(data = serializer_class.errors, safe = False)
